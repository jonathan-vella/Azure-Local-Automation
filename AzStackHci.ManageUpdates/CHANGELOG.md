...
- **Performance**: `Get-AzureLocalUpdateSummary` and `Get-AzureLocalClusterUpdateReadiness` now accumulate per-cluster results in a `[System.Collections.Generic.List[object]]` (O(1) amortised `.Add()`) instead of an `Object[]` with `+=` (O(n) per append, O(n^2) total). Measurable speed-up at fleet scale (1000+ clusters) for the post-shard merge step; no API surface change - the functions still return arrays.
