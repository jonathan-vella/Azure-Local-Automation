...
function Invoke-AzResourceGraphQuery {
    ...
    while ($true) {
        ...
        $exit = $LASTEXITCODE
        if ($exit -ne 0) {
            throw "Azure Resource Graph query failed (exit $exit): $(($raw | Out-String).Trim())"
        }
        ...
        try {
            ...
            throw "Azure Resource Graph query failed to parse JSON: $($_.Exception.Message); raw: $($rawText.Substring(0, [Math]::Min(500, $rawText.Length)))"
        }
