...
<#
.SYNOPSIS
    Runs Pester tests and generates HTML report for the AzLocal.DeploymentAutomation module.
...
Write-Host "AzLocal.DeploymentAutomation - Pester Tests" -ForegroundColor Cyan
Write-Host "======================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($useLogFile) {
    # Step 1: Run Pester with detailed output redirected ONLY to the log file.
    # This prevents VS Code terminal from hanging on large test suites.
    # We use *> to redirect ALL streams (output, error, warning, info) to file only.
    $logConfig = New-PesterConfiguration
    $logConfig.Run.Path = $PSScriptRoot
    $logConfig.Run.PassThru = $false  # No result object needed from this run
    $logConfig.TestResult.Enabled = $false  # NUnit XML saved in the second run
    ...
    Invoke-Pester -Configuration $logConfig *> $logPath
    Write-Host "Detailed test output saved to: $logPath" -ForegroundColor Green

    # Step 2: Run Pester again silently to get the PassThru result object for HTML report.
    # With Verbosity = None this is fast and produces no console output.
    Write-Host "Generating results summary..." -ForegroundColor Gray
...
<head>
    ...
    <title>AzLocal.DeploymentAutomation - Test Results</title>
    <style>
        :root {
...
<body>
    <div class="container">
        <header>
            <h1>&#x1F9EA; AzLocal.DeploymentAutomation</h1>
...
$moduleVersion = (Get-Module AzLocal.DeploymentAutomation -ErrorAction SilentlyContinue).Version
[void]$htmlBuilder.Append(@"
            </div>
        </div>

        <footer>
            <p>Generated by AzLocal.DeploymentAutomation Pester Tests</p>
