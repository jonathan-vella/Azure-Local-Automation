function Get-AzureLocalUpdateRunFailures {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        Today the existing `Get-AzureLocalUpdateRuns` cmdlet returns the
    ...
    .EXAMPLE
        ...
        Get-AzureLocalUpdateRunFailures -View Summary

    .EXAMPLE
        # Currently unresolved failures only (no later Succeeded run for
        # the same cluster+update), in the Prod ring, last 90 days
        Get-AzureLocalUpdateRunFailures -UpdateRingTag Prod -Since (Get-Date).AddDays(-90) -OnlyUnresolved

    .EXAMPLE
        # Single cluster, raw progress JSON for deep-dive investigation
        Get-AzureLocalUpdateRunFailures -ClusterName Arizona -IncludeRawProgress |
    ...
    .EXAMPLE
        ...
        $detail  = Get-AzureLocalUpdateRunFailures -View Detail  -ExportPath .\reports\update-failures-detail.json  -PassThru
        $summary = Get-AzureLocalUpdateRunFailures -View Summary -ExportPath .\reports\update-failures-summary.csv  -PassThru
