...
"@
    ...
    [void]$sb.AppendLine('')
    [void]$sb.AppendLine('# ---- AllowedUpdateVersions (schema v2, optional) ------------------')
    [void]$sb.AppendLine('# >>> ALLOWED-UPDATE-VERSIONS-V2 <<<')
    [void]$sb.AppendLine('# OPTIONAL fleet-wide allow-list of Azure Local solution-update names')
    [void]$sb.AppendLine("# or version strings. When set, Step.6 (apply-updates) only installs")
    [void]$sb.AppendLine("# updates whose 'name' OR 'properties.version' is an EXACT (case-")
    [void]$sb.AppendLine('# insensitive) match for one of the entries. If a cluster has no Ready')
    [void]$sb.AppendLine('# update that matches, that cluster is SKIPPED with status')
    [void]$sb.AppendLine("# 'NotInAllowList' (strict no-op; never falls back to 'latest').")
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine("# Typical use: 'minimum updates' policy - only YY04 + YY10 feature")
    [void]$sb.AppendLine('# updates plus the preceding cumulative updates per year.')
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine("# Format: semicolon-separated string (same convention as 'rings:').")
    [void]$sb.AppendLine("# Per-row override: set 'allowedUpdateVersions:' on any schedule row")
    [void]$sb.AppendLine('# below. Per-row beats top-level; multiple matching rows UNION their')
    [void]$sb.AppendLine('# lists; rows without the field on a UNION day are treated as')
    [void]$sb.AppendLine("# 'no opinion' (not 'allow nothing').")
    [void]$sb.AppendLine('#')
    [void]$sb.AppendLine('# Uncomment + edit to enable. Leave commented (or omit entirely) to')
    [void]$sb.AppendLine("# keep the default behaviour of 'install the latest Ready update'.")
