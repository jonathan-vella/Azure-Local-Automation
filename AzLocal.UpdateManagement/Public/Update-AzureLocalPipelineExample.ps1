function Update-AzureLocalPipelineExample {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        Companion to Copy-AzureLocalPipelineExample. Where Copy is a CLEAN
        ...
        Safety:
          - The destination is required to already exist - this is an UPDATE
            tool. Use Copy-AzureLocalPipelineExample for the initial drop.
    ...
    .EXAMPLE
        Update-AzureLocalPipelineExample -Destination .\pipelines -Platform AzureDevOps -PassThru |
    ...
    .EXAMPLE
        Update-AzureLocalPipelineExample -Destination .\.github\workflows -Platform GitHub -Force
    ...
    .NOTES
        ...
        Added in    : v0.7.68
        See also    : Copy-AzureLocalPipelineExample (clean-overwrite tool)
    #>
    ...
    #    pattern used by Copy-AzureLocalPipelineExample so a side-by-side
    ...
    if (-not (Test-Path -LiteralPath $platformSrc -PathType Container)) {
        throw "Update-AzureLocalPipelineExample: bundled $Platform pipeline source folder not found at '$platformSrc'. The module install may be corrupt or this is a development checkout without the sample folder."
    }
    ...
    if (-not (Test-Path -LiteralPath $Destination -PathType Container)) {
        throw "Update-AzureLocalPipelineExample: -Destination folder '$Destination' does not exist. Use Copy-AzureLocalPipelineExample for the initial drop, then re-run Update from the same folder."
    }
    $destResolved = (Resolve-Path -LiteralPath $Destination -ErrorAction Stop).ProviderPath

    Write-Log -Message "Update-AzureLocalPipelineExample: comparing bundled $Platform samples in '$platformSrc' against destination '$destResolved'." -Level Info
    ...
    if ($srcFiles.Count -eq 0) {
        Write-Log -Message "Update-AzureLocalPipelineExample: no source *.yml files found under '$platformSrc'." -Level Warning
        return
    }
    ...
    Write-Log -Message "Update-AzureLocalPipelineExample: $($results.Count) source file(s) processed - $summary" -Level Info
