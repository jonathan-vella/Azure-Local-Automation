function New-AzLocalFleetStatusHtmlReport {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        - Get-AzLocalClusterInventory (cluster list and UpdateRing tags)
        - Get-AzLocalClusterUpdateReadiness (health state, readiness)
        - Get-AzLocalUpdateSummary (current update versions)
        - Get-AzLocalAvailableUpdates (pending updates)
        - Get-AzLocalUpdateRuns (recent update history, optional)
        - Test-AzLocalClusterHealth (detailed health checks, optional)
    ...
    .EXAMPLE
        New-AzLocalFleetStatusHtmlReport -AllClusters -OutputPath "C:\Reports\fleet-all.html"
        Generates an HTML report for all clusters (up to 100) across the subscription.
    
    .EXAMPLE
        New-AzLocalFleetStatusHtmlReport -ScopeByUpdateRingTag -UpdateRingValue "Wave1" -OutputPath "C:\Reports\wave1-status.html"
        Generates an HTML report for all Wave1 clusters.
    
    .EXAMPLE
        New-AzLocalFleetStatusHtmlReport -ClusterNames @("Cluster01","Cluster02") -OutputPath "C:\Reports\fleet.html" -IncludeHealthDetails -IncludeUpdateRuns
        Generates a full report with health details and update run history.
    
    .EXAMPLE
        $html = New-AzLocalFleetStatusHtmlReport -ScopeByUpdateRingTag -UpdateRingValue "Production" -OutputPath "C:\Reports\prod.html" -PassThru
    ...
    if ($StatusData) {
        ...
        # Collect data via Get-AzLocalFleetStatusData (sequential single-pass)
        $collectParams = @{}
        ...
        $StatusData = Get-AzLocalFleetStatusData @collectParams
        if (-not $StatusData) {
