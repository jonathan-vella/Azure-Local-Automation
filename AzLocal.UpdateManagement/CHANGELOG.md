...
- **`Automation-Pipeline-Examples/README.md` now documents the default triggers and schedules for all six shipped pipelines.** A new "Default triggers and schedules (at a glance)" table at the top of Appendix A lists the GitHub Actions and Azure DevOps trigger / cron for every pipeline (`inventory-clusters`, `manage-updatering-tags`, `assess-update-readiness`, `apply-updates`, `fleet-update-status`, `fleet-health-status`). Each of the per-pipeline appendix entries (A.1 - A.6) now also has a dedicated **Trigger** row. **A.6 (Fleet Health Status)** is added in this release. **Apply Updates (A.4) and section 8 now include a mandatory-customisation callout**: the cluster `UpdateWindow` / `UpdateExclusions` tags only *gate* updates while the pipeline is already running; they do **not** start the pipeline. If `apply-updates.yml` is left with `workflow_dispatch` only (GH) / `trigger: none` (ADO) and you rely on `UpdateWindow` tags, no updates will ever be applied automatically. Section 8 includes worked GH / ADO cron examples for typical `UpdateWindow` values.
