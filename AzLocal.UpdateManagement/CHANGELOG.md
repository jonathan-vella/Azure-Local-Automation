...
- **All five `Describe` blocks that were `-Skip`-marked in the v0.7.68 ARG-first refactor have been un-skipped and rewritten against `Invoke-AzResourceGraphQuery` mocks.** 10 tests now pass (FleetProgress: 2, UpdateSummary: 2, ClusterUpdateReadiness multi-cluster: 1, ClusterUpdateReadiness readiness gates: 4, Get-AzureLocalUpdateRuns parallel dispatch: 1). The new mock pattern uses `InModuleScope` + a `function global:az { ... }` shim + `Mock Test-AzCliAvailable` / `Mock Install-AzGraphExtension` / `Mock Invoke-AzResourceGraphQuery` returning rows shaped per each cmdlet's KQL `project` clause (`ClusterResourceId_`, `properties` bag matching the ARM REST shape). Full suite: **Passed=504, Failed=0, Skipped=0**.
